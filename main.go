...
func buildRouter(cfg config.Config, postgresStore *postgresstore.Store, redisStore *redisstore.Store) *gin.Engine {
	...
	repositories.PATCH("/:id", authMiddleware.Optional(), repositoryController.Update)
	...
	deleteRequests.POST("/:id/reject", repositoryController.RejectDeleteRequest)
	deleteRequests.DELETE("/:id", repositoryController.CompleteDeleteRequest)
